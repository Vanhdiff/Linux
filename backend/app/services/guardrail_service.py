...
class GuardrailService:
    ...
    def pre_trade_validate(self, payload: PreTradeValidationRequest) -> dict:
        status = self.trade_block_status(payload.account_id)
        ...
        return {
            ...
            "reason": self._pre_trade_denial_reason(status) if blocked else None,
            "trade_blocking_enabled": status["trade_blocking_enabled"],
            "reasons": status["reasons"],
            "block_state": status["block_state"],
    ...
    def resolve_block(self, account_id: int) -> dict:
        ...
        self._ensure_account(account_id)
        return self._block_pipeline.resolve_block(account_id)

    def block_state(self, account_id: int) -> dict:
        """Return lightweight block status for the frontend."""
        self._ensure_account(account_id)
        return self._block_pipeline.get_block_state(account_id)

    def _pre_trade_denial_reason(self, status: dict) -> str:
        reasons = status.get("reasons") or []
        if not reasons:
            return "Trading is blocked by current guardrail state."
        return str(reasons[0].get("message") or "Trading is blocked by guardrail rules.")
    ...
    def _daily_loss_check(
        ...
        message = (
            ...
            if triggered
                ...
                f"Closed PnL {closed_pnl} plus floating PnL {floating_value} "
                f"has not reached max daily loss {threshold}."
            )
        )
    ...
    def _daily_profit_check(
        ...
        message = (
            ...
            if triggered
            else f"Daily PnL {pnl} has not reached max daily profit {threshold}."
        )
        return self._check_payload(
    ...
    def _trade_block_payload(self, enabled: bool, reasons: list[dict]) -> dict:
        ...
        if reasons and not enabled:
            disabled_reason = "trade_blocking_disabled"
        return {
            "enabled": enabled,
            "blocked": blocked,
            "reasons": reasons,
            "reason_count": len(reasons),
            "blockable_reason_count": len(reasons),
