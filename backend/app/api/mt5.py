...
from app.application.mt5_protection import EACommunicationLayer, EAStatus
...
def mt5_ea_setup_report(
    ...
    setup_manager = Mt5EASetupRepairService()
    blocker = getattr(request.app.state, "mt5_trade_blocker", None)
    ...
    setup_state = setup_manager.build_setup_state(
        installer_status=installer_status,
        ea_status=EAStatus(
            connected=bool(ea.get("connected")),
            last_heartbeat=None,
            version=ea.get("version", "") or "",
            account_id=ea.get("account_id"),
            error=ea.get("error", "") or "",
            stale=bool(ea.get("stale")),
        ),
        diagnostics=diagnostics,
        protection_level=protection_status.get("level"),
        backend_blocker_running=protection_status.get("backend_blocker_running"),
    )
    ready = (
